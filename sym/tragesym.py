...
official_attr = ["symversion", "device", "refdes", "value", "footprint", "graphical",
                "numslots", "slot", "slotdef","description", "comment", "author",
                "documentation", "dist-license", "use-license"]
single_attr_warning = ["symversion", "device", "footprint", "graphical", "author", "documentation",
...
P_NR, P_SEQ, P_TYPE, P_STYLE, P_POS, P_NET, P_LABEL, P_SWAP = 0,1,2,3,4,5,6,7
...
class Pin:
    ...
    def __init__(self, element):
        ...
        self.swap = element[P_SWAP].strip()
...
### Count the number of pins in each side
    ...
    bottomleftx, bottomlefty = pinlength + 100, 200
    if numpbottom > 0:
...
### decide which pindirection to use
        ...
        if pin.pos == "l": #left pin
            ...
            pint=(175,50,6,0) # dx, dy, alignment, angle
            pinl=(250,0,1,0)  # """"
            pina=(250,0,2,0)  # """"
            pinq=(175,-50,8,0)  # """"
...
### draw pinlabel and pintype
        ...
        if o_wordswap=="no":
            if pin.swap=="s":
                ...
                label=pin.label
        f.write("T %i"%pinlx+" %i"%pinly+" 9 10 1 1 %i"%pinlr+" %i 1\n"%pinla)
        f.write("pinlabel="+label+"\n")
        f.write("T %i"%pinax+" %i"%pinay+" 5 10 0 1 %i"%pinar+" %i 1\n"%pinaa)
...
#    urefx, urefy = bottomleftx+o_symwidth, bottomlefty + high + 100
    urefx, urefy = bottomleftx, bottomlefty + high + 50
...
#    else:
    ...
    for a in single_attr_warning:
        if attr.has_key((a,1)):
            if (a == "graphical" and attr[("graphical",1)] == "0"):
                continue
